...
export default defineConfig({
  ...
  webServer: [
    {
      // The API. Fastify runs from the api/ workspace, no DB needed
      // because the in-memory seed has all40 cities.
      command: 'npm -w @relocatewise/api run dev',
      cwd: '..',
    ...
    },
    {
      ...
      command: `npm -w @relocatewise/web run build && npm -w @relocatewise/web run preview -- --port ${PORT} --strictPort`,
      cwd: '..',
      url: BASE_URL,
      reuseExistingServer: !process.env.CI,
      timeout: 120_000,
