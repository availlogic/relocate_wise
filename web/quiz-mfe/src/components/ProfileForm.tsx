 ...
 * questionnaire (Quiz MFE).
 *
 * Implements PRD §5 (FR-1, FR-2, FR-3) and Acceptance-Criteria
 * Feature 2:
 ...
 *   - All fields are optional — a skipped question is recorded as
 *     the neutral default per Architecture §6.3.
 *   - On submit, the form:
 *       1. Posts the assembled `UserProfile` to `POST /api/match`.
 *       2. Caches the response in sessionStorage.
 *       3. Dispatches a `rw:quiz_completed` Custom Event on `window`
 *          with the `UserProfile` data payload (Architecture §4.1,
 *          FTC-17, Acceptance-Criteria Feature 2).
 *       4. Navigates to `/results` (the container's React Router
 *          resolves the route; the Dashboard MFE's ResultsPage
 *          picks up the cached response).
 *
 * The Custom Event is the **documented** cross-MFE boundary. The
 * container listens for the event as a fallback so the route
 * resolves even if the dashboard chunk has not yet loaded.
 */
...
export function toUserProfile(state: WizardState): UserProfile {
  ...
  // The container's i18n module is the canonical source for the
  // active language. The MFE doesn't import the container's i18n
  // module directly to stay self-contained — we read the
  // `i18next.language` global that react-i18next exposes.
...
export function ProfileForm({ initial }: ProfileFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 1. Dispatch the Custom Event on `window` so the container
      //    and other MFEs can observe the submission (FTC-17,
      //    Architecture §4.1). The detail payload is the
      //    `UserProfile` so the dashboard MFE can re-issue the
      //    match API call if needed.
      try {
        if (typeof window !== 'undefined') {
          window.dispatchEvent(
            new CustomEvent('rw:quiz_completed', {
              detail: { profile, at: Date.now() },
            }),
          );
        }
      } catch {
        /* CustomEvent ctor can throw in some test envs — ignore */
      }

      // 2. Persist the profile in sessionStorage so the dashboard
      //    MFE can rehydrate after a back-navigation that drops
      //    the location state.
      try {
        sessionStorage.setItem('rw:profile', JSON.stringify(profile));
      } catch {
        /* sessionStorage may be unavailable */
      }

      // 3. Navigate to the results route. The container will
      //    resolve `/results` to the dashboard MFE's ResultsPage.
      navigate('/results');
    } catch (err) {
