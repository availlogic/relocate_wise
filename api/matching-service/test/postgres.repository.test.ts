...
describeMaybe('PostgresCityRepository', () => {
  ...
  it('PostGIS geom column is populated (architecture §5.1 forward-compat)', async () => {
    const res = await pool!.query<{ slug: string; geom: string | null }>(
      `SELECT slug, ST_AsText(geom) AS geom FROM matching.cities WHERE slug = $1`,
      ['lisbon-pt'],
    );
  ...
  it('schema segregation — tables live in the matching schema (Phase A)', async () => {
    const res = await pool!.query<{ table_schema: string; table_name: string }>(
      `SELECT table_schema, table_name
         FROM information_schema.tables
        WHERE table_schema IN ('matching', 'public')
          AND table_name IN ('cities', 'city_scores')
        ORDER BY table_schema, table_name`,
    );
    // Both tables must exist in the matching schema and must NOT be in
    // the public schema anymore.
    const schemas = new Set(res.rows.map((r) => `${r.table_schema}.${r.table_name}`));
    expect(schemas.has('matching.cities')).toBe(true);
    expect(schemas.has('matching.city_scores')).toBe(true);
    expect(schemas.has('public.cities')).toBe(false);
    expect(schemas.has('public.city_scores')).toBe(false);
  });

  it('schema segregation — ingestion.pipeline_logs exists (Phase A)', async () => {
    const res = await pool!.query<{ table_name: string }>(
      `SELECT table_name
         FROM information_schema.tables
        WHERE table_schema = 'ingestion' AND table_name = 'pipeline_logs'`,
    );
    expect(res.rowCount).toBe(1);
  });
});
