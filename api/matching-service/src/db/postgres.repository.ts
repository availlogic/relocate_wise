 ...
 *
 * The schema (Architecture §5.1, Database §3) normalizes the data into
 * a `matching.cities` table (one row per city) and a `matching.city_scores`
 * table (one row per dimension). All queries are schema-qualified. The
 * pool passed to the constructor is expected to be the **matching**
 * service pool (read+write on `matching.*`); cross-schema writes from
 * this class are impossible by design — the DB role rejects them.
 *
 * A single SQL query joins the two tables and rehydrates the `City`
 * shape the matching engine and the rest of the API consume.
 *
...
export class PostgresCityRepository implements CityRepository {
  ...
  async truncateAll(client?: PoolClient): Promise<void> {
    ...
    try {
      await c.query('TRUNCATE matching.city_scores, matching.cities RESTART IDENTITY CASCADE');
    } finally {
