/**
 * Boot-time seed: if the `matching.cities` table is empty, bulk-insert
 * the in-memory `SEED_CITIES` array. Idempotent — safe to call on every
 * boot.
 *
 ...
 *
 * Phase A (v1.0.0 GA): the seed runs through the **matching** service
 * pool (`getMatchingPool`) and writes to `matching.cities` /
 * `matching.city_scores`. The seed path is the only non-`matching`
 * service write to the `matching` schema — it is gated on the matching
 * role having INSERT/UPDATE on the schema, which is true.
 */
...
export async function seedIfEmpty(pool: Pool): Promise<SeedResult> {
  const count = (
    await pool.query<{ n: number }>(`SELECT COUNT(*)::int AS n FROM matching.cities`)
  ).rows[0]!.n;
  if (count > 0) {
 ...
 *
 * The seed CLI uses the **admin** pool because migrations are applied
 * alongside the seed, and only the superuser (the `POSTGRES_USER` role
 * in `docker-compose.yml`) can run CREATE SCHEMA / GRANT. Production
 * boot paths call `runMigrations` then `seedIfEmpty` through the
 * matching pool directly; the CLI is a convenience for local resets.
 */
export async function runSeedCli(): Promise<void> {
  const { getAdminPool, closePool } = await import('./pool.js');
