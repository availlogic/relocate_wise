#!/usr/bin/env node
/**
 * Redirect script for the api/ parent directory.
 *
 * The `api/` directory is the parent of 3 sibling npm workspaces
 * (matching-service, ingestion-service, gateway), not a workspace
 ...
 *   npm -w @relocatewise/matching-service run db:seed
 */
...
const lines = [
  ...
  '',
  `  ${bold('api/')} is the parent directory of ${siblings.length} sibling npm workspaces:`,
